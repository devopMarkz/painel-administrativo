...
export interface Link { id: number; uuid: string; codigoPublico: string; idDocumento: number; documento: { id: number; municipio: string; periodoFormatado: string; tipoImposto: { sigla: string } } | null; exigeSenha: boolean; expiresAt: string | null; revogado: boolean; dataCriacao: string; dataAtualizacao: string; urlCompleta: string; ativo: boolean }
